...
#[clap(about = "Leearning Rust VIA useful OSX Utils")]
...
fn enumerate_pid_task_pid() {
    ...
    for (pid, process) in s.processes() {
        ...
        if task != 0x0 {
            println!("{}", format!("[+] PID: {} Process Name: {} is VULNERABLE[+]", pid, process.name().yellow()).green());
        }
    }
}
...
unsafe fn bootstrap_checks(name: String) {
    ...
    if result != 0x0 {
        println!("{}", format!("Service Name:{} is not accessible", service_name.into_string().unwrap()).red());
    } else {
        println!(
            "{}", format!("Service Name: {} is accessible", service_name.into_string().unwrap()).green()
        );
    }
}
